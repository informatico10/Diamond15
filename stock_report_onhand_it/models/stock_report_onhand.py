...
class StockReportOnhand(models.Model):
	_name = 'stock.report.onhand'
	_description = 'Reporte de productos en mano'
	_auto = False
	...
	def show_moves(self):
		cadsql = """
			...
			where state not in ('draft','cancel','done') and sale_line_id is not null
			and origen.id != destino.id and pt.type= 'product'
			and stock_move.product_id = %d
			and origen.complete_name = '%s'
		...
		return {
			...
			'view_mode': 'tree,form',
			'domain':[('id','in',aids)],
			'views': [(self.env.ref('stock_report_onhand_it.view_stock_move_report_onhand_tree').id, 'tree')],
		}

	def init(self):
		tools.drop_view_if_exists(self._cr, 'stock_report_onhand')
		query = """
			CREATE or REPLACE VIEW stock_report_onhand AS (
				select 
				...
				JOIN uom_uom ON stock_move.product_uom = uom_uom.id
				JOIN uom_uom uomt ON uomt.id = pt.uom_id
				where stock_move_line.state in ('done') and sale_line_id is not null
				and origen.id != destino.id and pt.type= 'product'
				and destino.usage='internal'
				group by 
				stock_move.product_id,
				...
				JOIN uom_uom ON stock_move.product_uom = uom_uom.id
				JOIN uom_uom uomt ON uomt.id = pt.uom_id
				where stock_move.state in ('done') and stock_move.purchase_line_id is not null and pt.type= 'product'

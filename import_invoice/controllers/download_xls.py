	...
	def download_document(self,model,id, **kw):
		...
		if res.account_opt == 'default':
			if res.type_import in ('out_invoice','out_refund'):
				invoice_xls = request.env['ir.attachment'].search([('name','=','invoice_customer.xls')])
			...
			if res.type_import in ('out_invoice','out_refund'):
				invoice_xls = request.env['ir.attachment'].search([('name','=','invoice_with_account_customer.xls')])
				filecontent = invoice_xls.datas
